...
public class MySysAcadImpl implements MySysAcad {
	...
	public void inscribirAlumnoCursada(Docente d, Alumno a, Materia m, Integer cicloLectivo)
			throws InscribirAlumnoCursadaDbException, DocenteNoDictaMateriaException {
	...
	public Double promedioAprobados(Materia m) {
		...
		return m.getExamenes().stream().filter(e -> e.getMateria().equals(m)).filter(examenAprobado)
				.mapToDouble(e -> e.getNota()).average().getAsDouble();
	}

	@Override
	public List<Alumno> inscriptos(Materia m, Integer ciclo) {
		return m.getInscripciones().stream().filter(i -> i.getCicloLectivo().equals(ciclo)).map(i -> i.getInscripto())
				.sorted((a, b) -> a.getNombre().compareTo(b.getNombre())).collect(Collectors.toList());
